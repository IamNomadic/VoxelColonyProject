...
public class LiquidSimulator : MonoBehaviour
{
    ...
    // --- SOURCE API ---
    public void AddSource(Chunk c, int x, int y, int z) { waterSources.Add(new SourceNode { c = c, x = x, y = y, z = z }); }
    public void RemoveSource(Chunk c, int x, int y, int z) { waterSources.RemoveAll(n => n.c == c && n.x == x && n.y == y && n.z == z); }
    ...
    void WakeNeighborsInternalInInbox(Chunk c, int x, int y, int z)
    {
        AddToInbox(c, x + 1, y, z, 0); AddToInbox(c, x - 1, y, z, 0);
        AddToInbox(c, x, y + 1, z, 0); AddToInbox(c, x, y - 1, z, 0);
        AddToInbox(c, x, y, z + 1, 0); AddToInbox(c, x, y, z - 1, 0);
    }

    public void AddToSimulation(Chunk c, int x, int y, int z) { AddToInbox(c, x, y, z, 0); }
    ...
    void SimulateStep()
    {
        ...
        if (inboxNodes.Count > 0)
        {
            ...
            inboxNodes.Clear(); inboxUnique.Clear();
        }
    ...
    bool Flow(Chunk chunk, int x, int y, int z, int currentMoves, List<LiquidNode> nextList, HashSet<string> nextSet)
    {
        ...
        if (y > 0)
        {
            ...
            {
                BlockData below = belowC.GetBlock(bx, by, bz);
                byte levelBelow = (below == null) ? (byte)0 : belowC.GetFluidLevel(bx, by, bz);
    ...
    bool GetNeighborBlock(Chunk currentChunk, int x, int y, int z, out Chunk targetChunk, out int targetX, out int targetY, out int targetZ)
    {
        ...
        int nextCX = currentCoord.x; int nextCZ = currentCoord.y;
